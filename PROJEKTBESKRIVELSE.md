...
- **Vælg implantat (picks):** "+ Vælg implantat"-knap pr. kort (skifter til "✓ Valgt" når markeret) lægger implantatet i en delt in-memory liste (`Bryst.picks`) til brug i patientmodulernes printbare oversigt. Kun i hukommelsen — lukkes vinduet, er listen væk (jf. kernebeslutning 1). **Header-badge:** de valgte implantater vises altid i modulheaderen som produktnavn + volumen ("MemoryGel Cohesive I · Ultra High - 320 cc") — "2× navn" når samme implantat er valgt på begge sider, "Højre navn / Venstre navn" (hver på sin linje) ved forskellige — så det aldrig er i tvivl hvad der er valgt.
...
- **Asymmetri — mål og valg pr. side:** Toggle "Ens sider / Højre / Venstre" øverst under Mål. Ved per-side gemmes alle mål (bredde, højde, pinch, volumen, papil→IMF, kirtel-pinch) separat pr. side; shortlist-overskriften viser aktiv side, picks mærkes med side (samme implantat kan vælges til begge sider), og side fremgår af journaltekst, AK-ark og patientmodulernes oversigter/PDF'er. Første skift kopierer de fælles mål til begge sider. **Samlet H+V-AK-ark:** ved aktiv asymmetri fremkommer en knap "AK-markeringsark · begge sider" under side-vælgeren — når der er valgt et implantat på hver side, genererer den ét ark med begge siders implantat, mål og ptose-verdikt side om side (alle afledte mål vist som "H … / V …"). Et enkelt-side-ark bærer desuden en kontralateral-note (den anden sides mål + valgte implantat), så symmetrivurderingen ikke kræver to udskrifter holdt op mod hinanden.
...
## Evidensgrundlag (litteraturlag)

Et diskret lag der lader den, som vil dybere ned, se hvad litteraturen bag værktøjet faktisk siger — skrevet så en speciallæge får noget ud af det, uden at tage fokus fra selve værktøjet.

- **Diskret indgang pr. modul:** en lille "Evidensgrundlag"-linje med et ⓘ-ikon (nederst i Modul 2/3's sammenligning, under resultatnoterne i Modul 1, i katalogets hero) åbner et overlay i modulets egen palette. Fælles, modul-agnostisk motor (`Bryst.wireEvidence` + `--ev-*`-variabler sat via `data-ev`); indholdet er speciallæge-niveau referater + rene skemaer.
- **Samlet hub på forsiden:** en diskret men synlig pille-knap ("Evidensgrundlag — litteraturen bag værktøjet") mellem dørene og datagrundlags-stampen åbner en hub med fire farvekodede kort (ét pr. modul + katalog). Hvert kort åbner modulets fulde panel og vender tilbage til hub'en ved luk (`Bryst.openEvidence` med `_returnTo`).
- **Indhold pr. område:** Modul 1 — Tebbetts' bredderegel (pinch-korrektion), højde/projektion, LVC-geometrien og ekspanderforslaget. Modul 2 — Regnault-gradering, Hedéns 2-cm-regel, mastopexi-teknikvalg, kombinationsrækkefølge, fedtoverførsel. Modul 3 — Blondeels 3-trins-analyse, ⅛/⅜-volumenreglen, forløbstidslinjen, DBCG's egnethedskriterier. Katalog — Mentors seks produktfamilier.
- **Billedpolitik (låst):** alle illustrationer er **rene skemaer og simple, originale linjediagrammer** (algoritme-akser, geometri, lagdelte diagrammer, tidslinjer, tabeller) — aldrig gengivelser af forlagsfigurer, aldrig patientfotos. `/grafik`/AI-genereret grafik blev afprøvet og fravalgt: OpenAI's moderation blokerer klinisk brystanatomi, og resultatet blev uskarpt/upræcist. Hvert panel afsluttes med kildeangivelse og noten om, at skemaerne er originale forenklinger, ikke kliniske protokoller.
